...
function App() {
    ...
    useEffect(() => {
        const setupGlobalShortcut = async () => {
            try {
                // Ctrl+Shift+U: Clipboard only (Linux-friendly shortcut)
                await register(
                    "CmdOrCtrl+Shift+U",
                    async (event: { state: string }) => {
                        if (event.state === "Pressed") {
                            console.log("⌨️ Ctrl+Shift+U pressed - clipboard only");
                            await handleToggle(false);
                        }
                    }
                );

                // Ctrl+Shift+Y: Clipboard + Screenshot
                await register(
                    "CmdOrCtrl+Shift+Y",
                    async (event: { state: string }) => {
                        if (event.state === "Pressed") {
                            console.log("⌨️ Ctrl+Shift+Y pressed - clipboard + screenshot");
        ...
        >
            ...
            >
                ...
                >
                    ...
                    ></div>
                    <div style={{ color: "#a1a1aa" }}>Ctrl+C to copy</div>
                </div>
